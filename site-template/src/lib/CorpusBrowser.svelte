<script lang="ts">
  // Shared renderer for a markdown corpus browser ([...path] route).
  //
  // Props:
  //   data         - LoadResult from $lib/corpus.ts
  //   prefix       - URL prefix for this corpus
  //                  (e.g. /mysteries/ancient-academy/i-ching/research)
  //   rootLabel    - breadcrumb label for the root segment ("Research", "MEV")
  //   rootBackHref - back-link target when at the root of the corpus
  //                  (i.e. the parent page outside this catch-all)

  import { marked } from 'marked';
  import MysteryPage from '$lib/MysteryPage.svelte';
  import type { LoadResult } from '$lib/corpus';
  ...
  let { data, prefix, rootLabel, rootBackHref }: Props = $props();

  // Title: last segment if any, else the root label.
  let title = $derived(
    data.segments.length ? data.segments[data.segments.length - 1] : rootLabel,
  );

  // Marked is synchronous; content is trusted (read-only mount), so
  // {@html} is acceptable.
...
<MysteryPage {title} {backHref}>
  ...
  {#if data.kind === 'md'}
    <!-- Trusted source: read-only corpus on disk. -->
